...
class PDF: 
  ...
  def __init__(self,mellin=None):
    ...
    self.resN1size=mell1.Np.size #--one contour for +M, full for -M
    self.resN2size=mell2.Np.size #--improvements based on M values should be made
    self.resN3size=mell3.Np.size #--one contour for -M, full for +M

    self.M=conf['dymell1'].M

    mell.N=np.append(mell.N,mell1.Np)
    mell.N=np.append(mell.N,mell2.Np)
    mell.N=np.append(mell.N,mell3.Np)
