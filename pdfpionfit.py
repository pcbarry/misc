...
class PDF:
  ...
  def set_params(self):
    """
    valence: q_v(x) = norm* x**a * (1-x)**b * (1+c*x**d)
    sea, gluon: f(x) = norm * x**a * (1-x)**b * (1+c*x**0.5+d*x)
    """
    params={}

    #--first shapes
    params['g1'] =np.array([1,-0.835058960119,8.514847173766,0,0])
    params['ubv1']=np.array([1,-0.272658307795,0.937614807355,0,0])
    params['dv1']=np.array([1,-0.272658307795,0.937614807355,0,0])
    params['u1']=np.array([0.026051033476,-0.716451078543,5.853785266721,0,0])
    params['db1']=np.array([0.026051033476,-0.716451078543,5.853785266721,0,0])
    params['s1'] =np.array([0.026051033476,-0.716451078543,5.853785266721,0,0])
    params['sb1']=np.array([0.026051033476,-0.716451078543,5.853785266721,0,0])
    ...
    self.params=params
    self.FLAV=['g1','ubv1','dv1','u1','db1','s1','sb1']
    self.PAR=['N','a','b','c','d']
  ...
  def set_sumrules(self):
    ...
    self.params['ubv1'][0]=1/self.get_moments('ubv1',1)

    self.params['dv1'][0]=1
    self.params['dv1'][0]=1/self.get_moments('dv1',1)

    #--msr
    up=2*self.get_moments('u1',2)+self.get_moments('ubv1',2)
    dp=self.get_moments('dv1',2)+2*self.get_moments('db1',2)
    sp=self.get_moments('s1',2)+self.get_moments('sb1',2)
    self.params['g1'][0]=1
  ...
  def set_moms(self):
    ...
    u   = self.get_moments('u1')
    ubv = self.get_moments('ubv1')
    ub  = ubv+u
    moms['up']=u + ub

    db = self.get_moments('db1')
    dv = self.get_moments('dv1')
    d  = dv+db
    moms['dp']=d+db

    s  = self.get_moments('s1')
    sb = self.get_moments('sb1')
    moms['sp']=s+sb
