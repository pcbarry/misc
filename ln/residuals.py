...
class RESIDUALS(_RESIDUALS):
    ...
    def _get_theory(self,entry):
        ...
        if obs=='F2LN (valence)':
            ...
            conf['pdf-pion'].storage[Q2]['qp'][1]=-conf['pdf-pion'].storage[Q2]['qm'][1]
            conf['pdf-pion'].storage[Q2]['qp'][2]=conf['pdf-pion'].storage[Q2]['qm'][2]
            for i in range(3,6):
                conf['pdf-pion'].storage[Q2]['qp'][i]=np.zeros(len(conf['pdf-pion'].storage[Q2]['qp'][i]))

            thy=conf['ln'].get_F2LN(xpi,y,Q2,kT2max)
            del conf['pdf-pion'].storage[Q2]

        elif obs=='F2LN (sea)':
            conf['pdf-pion'].evolve(Q2)
            conf['pdf-pion'].storage[Q2]['g']=np.zeros(len(conf['pdf-pion'].storage[Q2]['g']))
            conf['pdf-pion'].storage[Q2]['qp'][1]=2*conf['pdf-pion'].storage[Q2]['u']
            conf['pdf-pion'].storage[Q2]['qp'][2]=2*conf['pdf-pion'].storage[Q2]['db']
            thy=conf['ln'].get_F2LN(xpi,y,Q2,kT2max)
            del conf['pdf-pion'].storage[Q2]
            ...
            thy=conf['ln'].get_R(x,xpi,deltaxL,y,Q2,kT2max)
