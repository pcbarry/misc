...
try: from WaveFuncs.av18.av18 import av18
except: pass
try: from WaveFuncs.wjc.wjc import wjc
except: pass
try: from WaveFuncs.cdbonn.cdbonn import cdbonn
except: pass
import time
from WaveFuncs.wjc.wjc import wjc
...
class DEUTERON: 
    ...
    def __init__(self,path2nuctab='.',precalc=False,dsmf_type = 'paris'):
  
        self.path2nuctab=path2nuctab.rstrip('/')
        self.D = {}

        self.dsmf_type = dsmf_type
    ...
    def get_f(self,p):
        ...
        elif dsmf_type == 'wjc-1' or dsmf_type == 'wjc-2':
            if dsmf_type == 'wjc-1': u,w = wjc(p/self.hc,1)   
            if dsmf_type == 'wjc-2': u,w = wjc(p/self.hc,2)  

            #--Output wavefunctions in fm^3/2 => MeV^-3/2
            u/= self.hc**1.5
            w/= self.hc**1.5

        elif dsmf_type == 'cdbonn': 
            # Deuteron wave function (Momenta in MeV)
            # Nonrelativistic wave functions 
            u,w = cdbonn(p/self.hc)
        ...
        f1 = (u**2 - w**2/2.0)
        f2 = (3.0/2.0)*(w**2 - np.sqrt(2.0)*w*u)
    ...
    def integrand(self,X,gam,fXX,kind):
        ...
        if self.dsmf_type=='cdbonn' and p>1e6: p=1e6
        f0,f1,f2 = self.get_f(p) 
    ...
    def set_normalization(self):
        ...
        df01 = np.vectorize(lambda x0,x1:self.integrand([x0,x1],gam,fXX,kind))
        df1  = np.vectorize(lambda x1_: fixed_quad(lambda x0: df01(x0,x1_),0,1,n=1000)[0])
        self.norm = fixed_quad(lambda _x1_: df1(_x1_),0,1,n=200)[0]*(self.ymax-self.ymin)
    ...
    def gen_table(self,G,Y,fXX,kind):
        ...
        bar=BAR('computing %s-%s-%s'%(fXX,kind,self.dsmf_type),G.size*Y.size)
        ...
        for iG in range(G.size):
          for iY in range(Y.size): 
            ...
            bar.next()
        bar.finish()
        save(table,'%s/%s-%s-%s.dat'%(self.path2nuctab,fXX,kind,self.dsmf_type))
    ...
    def load_tables(self):
        ...
        for fXX in fXXs:
            for kind in ['onshell','offshell']:
                tab=load('%s/%s-%s-%s.dat'%(self.path2nuctab,fXX,kind,dsmf_type))
...
if __name__== "__main__":
    ...
    #deuteron=DEUTERON(path2nuctab='./grids/deuteron',precalc=True,dsmf_type='av18')
    #deuteron=DEUTERON(path2nuctab='./grids/deuteron',precalc=True,dsmf_type='wjc-1')
    #deuteron=DEUTERON(path2nuctab='./grids/deuteron',precalc=True,dsmf_type='wjc-2')
    deuteron=DEUTERON(path2nuctab='./grids/deuteron',precalc=True,dsmf_type='cdbonn')
