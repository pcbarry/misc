...
class PDF: 
  ...
  def __init__(self,mellin=None):
    ...
    self.resN1size=mell1.Nm.size #--one contour for -M, full for +M
    self.resN2size=mell2.Nm.size #--improvements based on M value should be made
    self.resN3size=mell3.Nm.size #--one contour for +M, full for -M

    self.M=conf['dymell1'].M

    mell.N=np.append(mell.N,mell1.Nm)
    mell.N=np.append(mell.N,mell2.Nm)
    mell.N=np.append(mell.N,mell3.Nm)
...
if __name__=='__main__':
  ...
  conf['aux']=AUX()
  conf['mellin']=MELLIN()
  conf['dymell1']=DYMELLIN(1)
  conf['dymell2']=DYMELLIN(2)
  conf['dymell3']=DYMELLIN(3)
