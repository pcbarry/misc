...
class PDF: 
  ...
  def __init__(self,mellin=None):
    ...
    if mellin==None:
      self.kernel=KERNELS(conf['mellin-kaon'],self.spl)
      self.dglap=DGLAP(conf['mellin-kaon'],conf['alphaS'],self.kernel,conf['dglap mode'],conf['order'])
  ...
  def set_params(self):
    ...
    params['sv1']=np.array([1, 0.5,3,0,0])
    params['sv2']=np.array([0, 0.5,3,0,0])
    params['sv3']=np.array([0, 0.5,3,0,0])
    params['u1']=np.array([1,-0.5,3,0,0])
    params['u2']=np.array([0,-0.5,3,0,0])
    params['u3']=np.array([0,-0.5,3,0,0])
    params['d1'] =np.array([0,-0.5,3,0,0])
    params['d2'] =np.array([0,-0.5,3,0,0])
    params['d3'] =np.array([0,-0.5,3,0,0])
  ...
  def set_sumrules(self):
    ...
    self.params['sv1'][0]=1
    self.params['sv1'][0]=(1-self.get_moments('sv2',1)-self.get_moments('sv3',1))/self.get_moments('sv1',1)
    ...
    d23  =                           self.get_moments('d2',2)+self.get_moments('d3',2)
    db23 =                           self.get_moments('db2',2)+self.get_moments('db3',2)
    dp23 = d23+db23

    sb23 =                           self.get_moments('sb2',2)+self.get_moments('sb3',2)
    sv   = self.get_moments('sv1',2)+self.get_moments('sv2',2)+self.get_moments('sv3',2)
    sp23 = 2*sb23 + sv
    ...
    self.sr['sv(1)']  = self.get_moments('sv1',1)+self.get_moments('sv2',1)+self.get_moments('sv3',1)
    self.sr['g(2)']   = g
    self.sr['sea(2)'] = 6*u 
    self.sr['valence(2)']=ubv+sv
  ...
  def set_moms(self):
    ...
    d  = self.get_moments('d1') +self.get_moments('d2')+self.get_moments('d3')
    db = self.get_moments('db1')+self.get_moments('db2')+self.get_moments('db3')
    moms['dp']=d+db

    sv = self.get_moments('sv1')+self.get_moments('sv2')+self.get_moments('sv3')
  ...
  def get_moments(self,flav,N=None):
    ...
    """
    if N==None: N=conf['mellin-kaon'].N
  ...
  def get_xF(self,x,Q2,flav,evolve=True):
    ...
    return x*conf['mellin-kaon'].invert(x,self.storage[Q2][flav])
  ...
  def get_second_moment(self,flav,Q2):
    ...
    if flav=='sea': 
      f=lambda x: 2*self.get_xF(x,Q2,'u',evolve=False)\
                 +self.get_xF(x,Q2,'d',evolve=False)\
                 +self.get_xF(x,Q2,'db',evolve=False)\
                 +2*self.get_xF(x,Q2,'sb',evolve=False)
    if flav=='ub valence':
      f=lambda x: self.get_xF(x,Q2,'ub',evolve=False)-self.get_xF(x,Q2,'u',evolve=False)
    if flav=='s valence':
      f=lambda x: self.get_xF(x,Q2,'s',evolve=False) -self.get_xF(x,Q2,'sb',evolve=False)
    return quad(f,0,1)[0]
