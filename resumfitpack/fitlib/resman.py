...
from qcdlib import pdfpion0,pdfpion1,pdfpionsea,pdfpionalt,wpdfalt,pdfpionaltexact,wpdfaltexact
...
try:
    ...
    import obslib.dy.dyres
    import obslib.dy.dyresmellin
    import obslib.dy.dyresexact
    import obslib.dy.dyresmellinexact
    import obslib.dy.dyresexactwestmark
except:
    pass
...
class RESMAN:
    ...
    def setup_core(self):
        ...
        if 'pdf-pion parametrization' in conf:
            ...
            if conf['pdf-pion parametrization']=='resum':
                conf['dymell1']=obslib.dy.dyresmellin.DYMELLIN(1)
                conf['dymell2']=obslib.dy.dyresmellin.DYMELLIN(2)
                conf['dymell3']=obslib.dy.dyresmellin.DYMELLIN(3)
                conf['Z']=74. #--Tungsten
                conf['A']=184. #--Tungsten
                conf['pdf-W']=wpdfalt.PDF()
                conf['pdf-pion']=pdfpionalt.PDF()
            if conf['pdf-pion parametrization']=='resum exact':
                conf['pdf-pion']=pdfpionsea.PDF()
                conf['Z']=74. #--Tungsten
                conf['A']=184. #--Tungsten
    ...
    def setup_wzrv(self):
        ...
        conf['wzrv']=obslib.wzrv.wzmellin.WZMELLIN()
    ...
    def setup_dy_pion(self):
        ...
        if conf['pdf-pion parametrization']=='resum': conf['resummed dy-pion']=obslib.dy.dyres.DYRES()
        if conf['pdf-pion parametrization']=='resum exact':
            q2=[]
            for k in conf['dy-pion tabs']:
                for i in range(len(conf['dy-pion tabs'][k]['Q2'])):
                    q2.append(conf['dy-pion tabs'][k]['Q2'][i])
            conf['double mellin resum']=obslib.dy.dyresmellinexact.DMELLIN(q2)
            #conf['pdf-pion']=pdfpionaltexact.PDF()
            conf['pdf-W']=wpdfaltexact.PDF()
            if 'westmark' in conf and conf['westmark']==True:
                conf['resummed dy-pion']=obslib.dy.dyresexactwestmark.DYRESEXACT()
            else:
                conf['resummed dy-pion']=obslib.dy.dyresexact.DYRESEXACT()
    ...
    def get_residuals(self,par):
        ...
        for chunk in results:
            for request in chunk:
                ...
                if request['reaction']=='dy-pion': #self.dy_pion_res.update_tabs_external(request)
                    if conf['pdf-pion parametrization']=='resum':
                        self.dy_pion_res.update_resum_tabs_external(request)
                    else:
                        self.dy_pion_res.update_tabs_external(request)
        ...
        if 'dy-pion' in conf['datasets']:
            ...
            rres=np.append(rres,out[1])
            nres=np.append(nres,out[2])
            res=np.append(res,self.parman.check_residual_penalty())
