...
class MAXLIKE:
    ...
    def get_residuals(self,par):
        ...
        #if self.cnt>5:
        #    print 'done after %s iterations'%self.cnt
    ...
    def run(self):
        ...
        if 'steps' in conf:
            ...
            for i in isteps:
                ...
                if 'tmc' in conf: msg+=', tmc = %s'%conf['tmc']
                if 'ht'  in conf: msg+=' , ht = %s'%conf['ht']
                if 'offshell' in conf: msg+=' , offshell = %s'%conf['offshell']
                print msg
                if 'ht' in conf and conf['ht']:
                    if conf['ht parametrization']==0: print('ht parametrization 0: additive (p/n)')
                    if conf['ht parametrization']==1: print('ht parametrization 1: additive (p/d)')
                    if conf['ht parametrization']==2: print('ht parametrization 2: multiplicative (p/d)')
                if 'offshell' in conf and conf['offshell']:
                    if conf['offshell parametrization']==0: print('offshell parametrization 0: polynomial (p/n) w/o sum rule')
                    if conf['offshell parametrization']==1: print('offshell parametrization 1: polynomial (p/n) w/ sum rule')
                    if conf['offshell parametrization']==2: print('offshell parametrization 2: factorized (p/n) w/ sum rule')
                    if conf['offshell parametrization']==3: print('offshell parametrization 3: factorized (p/n) w/o sum rule')
                if 'same ht' in conf and conf['same ht']: print('t4FL/t4F3 will be set equal to t4F2')
...
if __name__=='__main__':
    ...
    ap.add_argument('-n','--ncores',type=int,default=2,help='cpu cores')
