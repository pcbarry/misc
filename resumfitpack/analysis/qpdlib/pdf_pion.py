...
#from analysis.corelib import classifiermod as classifier
...
def gen_xf(wdir,Q2=None):
    ...
    if conf['pdf-pion parametrization']=='resum': conf['pdf-pion parametrization']='sea'
...
def plot_xf(wdir,kc,Q2=None):
    ...
    def plot_pdf(axl,axr,X,Y,ymin,ymax,factor=1,color='r'):
        ...
        for i in range(len(Y)):
            ...
            if nc>1: color=colors[cluster[i]]
    ...
    if Q2!=conf['Q20']: 
        py.savefig('%s/gallery/pdf-pion-%d.png'%(wdir,istep))
        #py.savefig('%s/gallery/pdf-pion-%d.pdf'%(wdir,istep))
    else: 
        py.savefig('%s/gallery/pdf-pion-Q20-%d.png'%(wdir,istep))
        #py.savefig('%s/gallery/pdf-pion-Q20-%d.pdf'%(wdir,istep))
    py.close()
...
def plot_mom1(wdir,kc,Q2=None):
    ...
    py.savefig('%s/gallery/pdf-mom-pion-%d.pdf'%(wdir,istep))
...
def gen_b_eff(wdir):
    ...
    print('\ngenerating effective b parameters from %s'%(wdir))
    ...
    for par in jar_replicas:
        ...
        parman.set_new_params(par,initial=True)
        if pdf.params['ubv1'][3]<0: continue

        func=lambda x: pdf.get_b_eff(x,'ubv1')# + pdf.get_b_eff(x,'ubv2') + pdf.get_b_eff(x,'ubv3')

        b_eff.append([func(x) for x in X])
    print     
    checkdir('%s/data'%wdir)
    save({'X':X,'b_eff':b_eff},'%s/data/b_eff-%d.dat'%(wdir,istep))

def plot_b_eff(wdir,kc):

    load_config('%s/input.py'%wdir)
    istep=core.get_istep()


    data=load('%s/data/b_eff-%d.dat'%(wdir,istep))
    ...
    for k in range(nc):
        ...
        for i in range(len(Y)):
            if k==cluster[i]:
                b[k].append(Y[i])
            #if nc>1: color=colors[cluster[i]]
            #ax.plot(X,np.array(Y[i]),color=color,ls='-',alpha=0.2)
        #mean=np.mean(Y,axis=0)
        #std=np.std(Y,axis=0)
        mean=np.mean(b[k],axis=0)
        std=np.std(b[k],axis=0)
        ax.fill_between(X,mean-std,mean+std,color=colors[k],alpha=0.3)
        ax.plot(X,mean,'%s--'%colors[k],lw=3)
    #ax.plot(X,mean-std,'k-',lw=3)
    #ax.plot(X,mean+std,'k-',lw=3)
    #ax.plot(X,mean,'k--',lw=3)

    ax.set_ylabel(r'$\bar{u}_{v}~{\rm effective}~\beta~{\rm parameter}$',size=20)
    ...
    py.tight_layout()
    checkdir('%s/gallery'%wdir)
    py.savefig('%s/gallery/b_eff-%d.png'%(wdir,istep))
    py.close()
