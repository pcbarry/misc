    ...
    for i in range(nc):
        ...
        for reaction in data:
            ...
            for idx in data[reaction]:
                repthy[i][reaction][idx]=[]
                for j in range(nrep):
                    if cluster[j]!=i: continue
                    cnt[i]+=1
                    repthy[i][reaction][idx].append(data[reaction][idx]['prediction-rep'][j])
                value=data[reaction][idx]['value']
                alpha=data[reaction][idx]['alpha']
                thy=np.mean(repthy[i][reaction][idx],axis=0)
                res=(value-thy)/alpha
                chi2ave[i]+=np.sum(res**2)

    chi2=[]
    for k in chi2ave:
        chi2.append(chi2ave[k])
