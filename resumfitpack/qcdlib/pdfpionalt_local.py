...
class PDF: 
  ...
  def __init__(self,mellin=None):
    ...
    self.resN1size=mell1.Npmin.size #--one contour for +M, full for -M
    self.resN2size=mell2.Np.size #--improvements based on M values should be made
    self.resN3size=mell3.Npmin.size #--one contour for -M, full for +M
  ...
  def set_sumrules_g(self):
      ...
      #--msr
      u   = self.get_moments('u1',2)+self.get_moments('u2',2)+self.get_moments('u3',2)
      ubv = self.get_moments('ubv1',2)+self.get_moments('ubv2',2)+self.get_moments('ubv3',2)
      up  = 2*u + ubv

      db = self.get_moments('db1',2)+self.get_moments('db2',2)+self.get_moments('db3',2)
      dv = self.get_moments('dv1',2)+self.get_moments('dv2',2)+self.get_moments('dv3',2)
      dp = dv + 2*db
      ...
      g2=self.get_moments('g2',2)
      g3=self.get_moments('g3',2)
      self.params['g1'][0]=1
      self.params['g1'][0]=(1-up-dp-sp-g2-g3)/self.get_moments('g1',2)
      g=self.get_moments('g1',2)+g2+g3
      msr=g+up+dp+sp
  ...
  def evolve(self,Q2):
    ...
    if Q2 not in self.storage:
        ...
        flav=['g','u','ub','d','db','s','sb','c','cb','b','bb','qp','qm']
        for f in flav:
            if f=='qp' or f=='qm':
                store=[]
                for j in range(len(moms[f])):
                    store.append(moms[f][j][:i0])
                self.storage[Q2][f]=store
            else:
        ...
        flav2=['g','u','ub','d','db','s','sb','c','cb','b','bb']
        for f in flav2:
...
if __name__=='__main__':
  ...
  conf['Q20'] = 1.0
  conf['aux']=AUX()
  conf['mellin-pion']=MELLIN(npts=8)
