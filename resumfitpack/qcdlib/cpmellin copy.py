...
class DMELLIN:
  ...
  def invert(self,F,G):
      ...
      return -np.real(np.sum(np.einsum('ij->i',H)*self.WN*self.JACN))/(2.*np.pi**2)
...
if __name__=='__main__':
  ...
  momM=1./(M+1)
  momNs=1./(Ns+1)
  momN=np.einsum('i,j->ji',momN,IM)
  momM=np.einsum('i,j->ji',IN,momM)
  momNs=np.einsum('i,j->ji',momNs,IM)
  f=lambda x: x
  g=lambda y: y

  xval=0.1
  yval=0.5
  F=momN*momM*(xval)**(-N)*yval**(-M)
  G=momNs*momM*(xval)**(-Ns)*yval**(-M)
  print mell.invert(F,G)
