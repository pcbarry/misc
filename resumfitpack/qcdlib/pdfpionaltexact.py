...
class PDF: 
  ...
  def __init__(self,mellin=None):
    ...
    if mellin==None:
        #self.kernel=KERNELS(conf['double mellin resum'],self.spl)
        #self.dglap=DGLAP(conf['double mellin resum'],conf['alphaS'],self.kernel,conf['dglap mode'],conf['order'])
        #self.mellin=conf['double mellin resum']
