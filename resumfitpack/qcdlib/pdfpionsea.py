...
class PDF: 
  ...
  def get_b_eff(self,x,flav):
    if flav not in self.params:
        print 'need another flavor, not in this class'
        sys.exit()
    M,a,b,c,d=self.params[flav]
    if 'v' in flav:
        b_eff=b-a*(1-x)/x-c*d*x**(d-1)*(1-x)/(1+c*x**d)
    else:
        b_eff=b-a*(1-x)/x-(0.5*c*(1-x)*x**(-0.5)+d*(1-x))/(1+c*x**0.5+d*x)
    return b_eff
