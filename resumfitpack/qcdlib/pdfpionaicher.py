...
class PDF: 
  ...
  def __init__(self,mellin=None):
    ...
    mell=copy.deepcopy(conf['mellin-pion'])
    ...
    self.resN1size=mell1.Npmin.size #--should be one contour for +M, full for -M
    self.resN2size=mell2.Np.size #--improvements based on M value should be made
    self.resN3size=mell3.Npmin.size #--should be one contour for -M, full for +M

    self.M=conf['dymell1'].M
    self.Nfullsize=len(conf['dymell1'].N)

    mell.N=np.append(mell.N,mell1.Npmin)
    mell.N=np.append(mell.N,mell2.Np)
    mell.N=np.append(mell.N,mell3.Npmin)
  ...
  def set_params(self):
    ...
    val:   norm * x**a * (1-x)**b * (1+c*x**d)
    sea,g: norm * x**a * (1-x)**b * (1+c*x**0.5+d*x)
    """
    params={}

    params['g1'] =np.array([5.90,-1+1.270,1.290,-2.074,1.824])
    params['g2'] =np.array([0,0,0,0,0])
    params['g3'] =np.array([0,0,0,0,0])
    params['ubv1']=np.array([0.838129163027,-1+0.70,2.03,13.8,2.0])
    params['ubv2']=np.array([0,0,0,0,0])
    params['ubv3']=np.array([0,0,0,0,0])
    params['dv1']=np.array([0.838129163027,-1+0.70,2.03,13.8,2.0])
    params['dv2']=np.array([0,0,0,0,0])
    params['dv3']=np.array([0,0,0,0,0])
    #params['u1']=np.array([0.352962*0.417,-1+0.207,4.454,-2.466,3.855])
    params['u1']=np.array([0.417,-1+0.207,4.454,-2.466,3.855])
    params['u2']=np.array([0,0,0,0,0])
    params['u3']=np.array([0,0,0,0,0])
    #params['db1']=np.array([0.352962*0.417,-1+0.207,4.454,-2.466,3.855])
    params['db1']=np.array([0.417,-1+0.207,4.454,-2.466,3.855])
    params['db2']=np.array([0,0,0,0,0])
    params['db3']=np.array([0,0,0,0,0])
    params['s1'] =np.array([0.000,0,0,0,0])
    params['s2'] =np.array([0.000,0,0,0,0])
    params['s3'] =np.array([0.000,0,0,0,0])
    params['sb1'] =np.array([0.000,0,0,0,0])
    params['sb2'] =np.array([0.000,0,0,0,0])
    params['sb3'] =np.array([0.000,0,0,0,0])
    self.params=params

  def set_sumrules(self):

    #--valence
    #--valence has 0.65 of momentum fraction
    momval=0.65
    vala,valb,vald=self.params['ubv1'][1],self.params['ubv1'][2],self.params['ubv1'][4],
    self.params['ubv1'][3]=self.get_valc(momval,vala,valb,vald)
    ...
    s=self.get_moments('s1',2)+self.get_moments('s2',2)+self.get_moments('s3',2)
    sb=self.get_moments('sb1',2)+self.get_moments('sb2',2)+self.get_moments('sb3',2)
    g=self.get_moments('g1',2)+self.get_moments('g2',2)+self.get_moments('g3',2)

    #self.params['u1'][0]=4
    self.params['u1'][0]=(1-up23-dp23-s-sb-g)/4
  ...
  def get_valc(self,mom,a,b,d):

    num=2*gamma(a+2)/gamma(a+b+3)-mom*gamma(a+1)/gamma(a+b+2)
    den=mom*gamma(a+d+1)/gamma(a+b+d+2)-2*gamma(a+d+2)/gamma(a+b+d+3)
    return num/den
  ...
  def evolve(self,Q2):
    ...
    if Q2 not in self.storage:
      ...
      for f in flav:
        res1=self.res_storage[(1,Q2)][f].reshape(shape)
        repeated1=np.einsum('i,j->ij',res1.T[-1],np.ones(len(M)/2-1))
        self.res_storage[(1,Q2)][f]=np.append(res1,repeated1,axis=1).flatten()
        res3=self.res_storage[(3,Q2)][f].reshape(shape)
        repeated3=np.einsum('i,j->ij',res3.T[0],np.ones(len(M)/2-1))
        self.res_storage[(3,Q2)][f]=np.append(repeated3,res3,axis=1).flatten()
  ...
  def get_pdfs(self,x,Q2,evolve=True):
    ...
    for flav in ['g','u','ub','d','db','s','sb','c','cb','b','bb']:
      pdfs.append(self.get_xF(x,Q2,flav))
    return pdfs/x
...
if __name__=='__main__':
  ...
  conf['Q20'] = 1.0
  conf['aux']=AUX()
  conf['mellin-pion']=DYMELLIN(1,npts=8)
