...
class _RESIDUALS:
    ...
    def setup_resum(self):
      self.percent_to_absolute()
      self.add_columns()
      self.get_alpha()
      self.retrieve_norm_uncertainty()
      self.setup_rparams()
      self.setup_resum_requests()
      self.original=copy.deepcopy(self.tabs)
      if 'bootstrap' in conf and conf['bootstrap']: self.resample()
    ...
    def setup_resum_requests(self):
        requests=[]
        for idx in self.tabs:
            Q2=[]
            for irow in range(len(self.tabs[idx]['value'])):
                if self.tabs[idx]['Q2'][irow] not in Q2: Q2.append(self.tabs[idx]['Q2'][irow])
            for q2 in Q2:
                request={}
                request['reaction'] = self.reaction
                request['dataset']  = idx
                request['irow']     = []
                request['thy']      = np.zeros(0)
                for irow in range(len(self.tabs[idx]['value'])):
                    if self.tabs[idx]['Q2'][irow]==q2:
                        request['irow'].append(irow)
                        request['thy']=np.append(request['thy'],0.0)
                    requests.append(request)
        self.requests=requests
    ...
    def update_resum_tabs_external(self,request):
        idx  = request['dataset']
        irow = request['irow']
        for i in range(len(irow)):
            self.tabs[idx]['thy'][irow[i]]=request['thy'][i]
