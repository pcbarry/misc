...
class RESIDUALS(_RESIDUALS):
    ...
    def get_F2p(self,x,Q2,stf='F2'):

        thy = 0

        #--leading twist

        thy+= self.stfuncs.get_FXN(x,Q2,stf,nucleon='proton',twist=2,Nf=self.Nf,tmc=self.tmc,evolve=False)
        ...
        if  self.ht==True: 
            ...
            thy+=self.stfuncs.get_FXN(x,Q2,stf,nucleon='proton',twist=4)
    ...
    def _get_theory(self,entry):
        ...
        elif obs=='FL'    and target=='p':  thy=self.get_F2p(x,Q2,stf=obs)
        elif obs=='F3'    and target=='p':  thy=self.get_F2p(x,Q2,stf=obs)
