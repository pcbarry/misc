...
class T4: 
  ...
  def set_params(self):
    params={}
    params['proton']  =np.array([0.0,0.5,3,0,0])
    params['deuteron'] =np.array([0.0,0.5,3,0,0])
    self.params=params
  ...
  def get_BC(self):
    ...
    self.H['deuteron']=self.get_moments('deuteron',self.mellin.N)

  def get_xF(self,x,Q2,nucleon):
    N,a,b,c,d = self.H[nucleon]
    return x*N*(x**a)*((1-x)**b)*(1+c*x**0.5+d*x) 
