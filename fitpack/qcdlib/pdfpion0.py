...
class PDF: 
  ...
  def set_params(self):
    ...
    params['dv2']=np.array([1, 0.5,3,0,0])
    params['u1']=np.array([1,-0.5,3,0,0])
    params['u2']=np.array([1,-0.5,3,0,0])
    params['db1']=np.array([1,-0.5,3,0,0])
    params['db2']=np.array([1,-0.5,3,0,0])
  ...
  def set_sumrules(self):
    ...
    self.params['ubv1'][0]=(1-self.get_moments('ubv2',1) )/self.get_moments('ubv1',1)

    self.params['dv1'][0]=1    
    self.params['dv1'][0]=(1-self.get_moments('dv2',1))/self.get_moments('dv1',1)

    #--msr
    u   = self.get_moments('u1',2)+self.get_moments('u2',2)
    ubv = self.get_moments('ubv1',2)+self.get_moments('ubv2',2)
    up  = 2*u + ubv

    db = self.get_moments('db1',2)+self.get_moments('db2',2)
    dv = self.get_moments('dv1',2)+self.get_moments('dv2',2)
    dp = dv + 2*db

    s  = self.get_moments('s1',2) +self.get_moments('s2',2)
    sb = self.get_moments('sb1',2)+self.get_moments('sb2',2)
    sp = s+sb   


    g2=self.get_moments('g2',2)
    self.params['g1'][0]=1    
    self.params['g1'][0]=(1-up-dp-sp-g2)/self.get_moments('g1',2)
    g=self.get_moments('g1',2)+g2
    msr=g+up+dp+sp

    #--share
    self.sr={}
    self.sr['msr']=msr
    self.sr['ubv(1)'] = self.get_moments('ubv1',1)+self.get_moments('ubv2',1)
    self.sr['dv(1)']  = self.get_moments('dv1',1)+self.get_moments('dv2',1)
    self.sr['g(2)']   = g
    self.sr['sea(2)'] = 6*u 
  ...
  def set_moms(self):
    ...
    u   = self.get_moments('u1')+self.get_moments('u2')
    ubv = self.get_moments('ubv1')+self.get_moments('ubv2')
    ub  = ubv+u
    moms['up']=u + ub

    db = self.get_moments('db1')+self.get_moments('db2')
    dv = self.get_moments('dv1')+self.get_moments('dv2')
    d  = dv+db
    moms['dp']=d+db

    s  = self.get_moments('s1') +self.get_moments('s2')
    sb = self.get_moments('sb1')+self.get_moments('sb2')
    moms['sp']=s+sb   
