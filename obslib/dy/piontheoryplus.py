...
class DY_PION_PLUS:
  ...
  def Cqqb0(self,f):
    return 0#(1+self.iord*self.CF*self.aS/np.pi*(3/2.*np.log(self.Q2/self.muF2)+2*np.pi**2/3-6))*0.5*(f(0)+f(1))
  ...
  def Cqqb2(self,z,y,f):
    ...
    return 0#self.iord*self.CF*self.aS/np.pi\
  ...
  def Cqg1(self,z): 
    return 0#self.iord*self.TF*self.aS/2/np.pi\
