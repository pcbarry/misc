...
class DY_PION:
  ...
  def load_nn_hybrid(self):
    ...
    for part in ['qA,qbB','qbA,qB','qA,gB','gA,qB']:
      ...
      for f in flavs:
          fname='%s/dy-pion/%s/%s.json'%(path2nntab,part,f)
          nn=load(fname)
          self.nn[part][f]={}
  ...
  def get_xsec_mell_hybrid(self,k,i,Q2,part,flav=None):
    ...
    #checkdir('/Users/barryp/JAM/workspace/ml4jam/dy/data')
    ...
    if part!='full': 
        ...
        checkdir('%s/dy/data/%s/%s/%s'%(conf['path2ml4jam'],k,part,flav))
        save(data[part][flav],'%s/dy/data/%s/%s/%s/%smell.dat'%(conf['path2ml4jam'],k,part,flav,i))
        ...
        checkdir('%s/dy/data/%s/%s'%(conf['path2ml4jam'],k,part))
        save(xsec,'%s/dy/data/%s/%s/%sxsecmell.dat'%(conf['path2ml4jam'],k,part,i))
  ...
  def get_xsec_mell_hybrid_nn(self,k,i,Q2,part,flav=None):
    ...
    Q2=np.ones(len(N))*Q2
    S=np.ones(len(N))*conf['dy-pion tabs'][k]['S'][i]
    ReN=N.real
    ImN=N.imag
    #x_vals=np.array([xF,ReN,ImN]).T
    x_vals=np.array([xF,Q2,S,ReN,ImN]).T
    ...
    if part!='full':
        ...
        #reresult=result[0]*np.cos(result[1])
        #imresult=result[0]*np.sin(result[1])
        #reresult=np.exp(result[0])*result[1]
        #imresult=np.exp(result[0])*result[2]
        reresult=result[0]*result[1]
        imresult=result[0]*result[2]
        #data[part][flav]=result[0]+1j*result[1]
        data[part][flav]=reresult+1j*imresult
        checkdir('%s/dy/data/%s/%s/%s'%(conf['path2ml4jam'],k,part,flav))
        save(data[part][flav],'%s/dy/data/%s/%s/%s/%snn.dat'%(conf['path2ml4jam'],k,part,flav,i))
        save(result,'%s/dy/data/%s/%s/%s/%snn_rcossin.dat'%(conf['path2ml4jam'],k,part,flav,i))
    ...
    if part!='full': xsec=mult*data[part][flav]
        ...
        if Nf>4: 
          xsec+= self.eD2*PDFA['b']  * data['qA,qbB']['bb']\
                ...
                +self.eD2*PDFA['g']  * data['gA,qB']['bb']
        checkdir('%s/dy/data/%s/%s'%(conf['path2ml4jam'],k,part))
        save(xsec,'%s/dy/data/%s/%s/%sxsecnn.dat'%(conf['path2ml4jam'],k,part,i))
