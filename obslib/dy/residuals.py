...
from obslib.dy.piontheorynonplus import DY_PION_NON_PLUS
...
class RESIDUALS_PIONS(_RESIDUALS):

  def __init__(self):
    self.reaction='dy-pion'
    self.tabs=conf['dy-pion tabs']
    if conf['pdf-pion parametrization']=='resum': self.setup_resum()
    else: self.setup()

  def _get_theory(self,entry): 
    if type(entry[1])!=int:
        return self._get_resum_theory(entry)
    k,i=entry
    Q2    =self.tabs[k]['Q2'][i]
    Jac   =self.tabs[k]['Jac'][i]
    units =self.tabs[k]['Units'][i]
    ...
    if obs == 'dsig/drtau/dxF (valence)':
        ...
        for f in ['g','u','db','s','sb','c','cb','b','bb']:
            conf['pdf-pion'].storage[Q2][f]=np.zeros(len(conf['pdf-pion'].storage[Q2][f]))
        thy1=conf['dy-pion'].get_xsec_mell_hybrid(k,i,Q2) * Jac * units
        del conf['pdf-pion'].storage[Q2]
        ...
        conf['pdf-pion'].evolve(Q2)
        conf['pdf-pion'].storage[Q2]['g']=np.zeros(len(conf['pdf-pion'].storage[Q2]['g']))
        conf['pdf-pion'].storage[Q2]['ub']=conf['pdf-pion'].storage[Q2]['u']
        conf['pdf-pion'].storage[Q2]['d']=conf['pdf-pion'].storage[Q2]['db']
    ...
    elif obs == 'dsig/drtau/dxF (non-plus)':
        thy = conf['dy-pion non-plus'].get_xsec_mell_hybrid(k,i,Q2) * Jac * units
  ...
  def _get_resum_theory(self,entry): 
    k,array=entry
    Q2=self.tabs[k]['Q2'][array[0]] #--Q2 is the same for elements of array
    S=self.tabs[k]['S'][array[0]] #--S is the same for elements of array
    units=self.tabs[k]['Units'][array[0]] #--units are the same for elements of array
    Y=[]
    Jac=[]
    for i in array:
      Y.append(self.tabs[k]['Y'][i])
      Jac.append(self.tabs[k]['Jac'][i])

    thy=np.zeros(len(array))
    for i in range(len(array)):
      thy[i]=conf['dy-pion'].get_xsec_mell_hybrid(k,array[i],Q2) * Jac[i] * units
    resum=conf['resummed dy-pion'].get_interpolated(S,Y,Q2,Q2) * np.array(Jac) * units
    fullthy=thy+resum
    return fullthy
