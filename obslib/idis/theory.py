...
class STFUNCS(AUX):
    ...
    def get_smf(self,x,Q2,target,twist,tmc,offshell,conj,fXX,proj):
        ...
        #--need to have x**(-M). Do not use xi even for TMCs. 
        if offshell==True:
            ...
            if twist==2 and proj!=None:    # add TMC
                if tmc=='GP':
                    if 'f2' in fXX or 'fL' in fXX: func=lambda N,M,x: self.get_TMC_GP(N,x,Q2,proj)*x**(-M)
                    else:                          func=lambda N,M,x: self.get_TMC_GP(N,x,Q2,proj)*x**(-M)
                
                if tmc=='AOT':
                    if 'f2' in fXX or 'fL' in fXX: func=lambda N,M,x: self.get_TMC_AOT(N,x,Q2,proj)*x**(-M)
                    else:                          func=lambda N,M,x: self.get_TMC_AOT(N,x,Q2,proj)*x**(-M)
              
            elif twist==2 and proj==None:  # no TMC
                if 'f2' in fXX or 'fL' in fXX:  func=lambda N,M,x: x**(-N+1-M)
    ...
    def get_T4FX(self,stf,nucleon,x,Q2):
        ...
        #--proton/deuteron can use x-space result
        if self.ht==1 or self.ht==2: 
            if stf=='F2':   N,a,b,c,d = self.t4F2.H[nucleon]
            elif stf=='FL': N,a,b,c,d = self.t4FL.H[nucleon]
            elif stf=='F3': N,a,b,c,d = self.t4F3.H[nucleon]
    ...
    def get_FXN(self,x,Q2,stf='F2',twist=2,nucleon='proton',Nf=None,tmc=False,precalc=False,evolve=True):
        ...
        if twist==4: 
          if self.ht==0: FX = x**(-self.mellin.N)*self.get_T4FX(stf,nucleon,x,Q2)
            ...
            if nucleon=='proton':  return self.get_T4FX(stf,nucleon,x,Q2)
            if nucleon=='neutron': return 0
          elif self.ht==2:
            if nucleon=='proton':  return self.get_FXN(x,Q2,stf=stf,twist=2,nucleon=nucleon,tmc=tmc,evolve=True)*self.get_T4FX(stf,nucleon,x,Q2)
            if nucleon=='neutron': return 0
    ...
    def get_F2D(self,x,Q2,twist=2,Nf=None,tmc=False,nuc=False,offshell=False,evolve=True): 
        ...
        if offshell==False:
            ...
            if twist==2:
                ...
                if    tmc==False and nuc==False: f22 = x**(-self.mellin.N)
            ...
            if twist==4:
                ...
                if self.ht==0:
                    ...
                    if nuc==False : f22=x**(-self.mellin.N)
                    else          : f22=self.tables[self.get_tname('4',fXX='f22',proj=None)].retrieve(x,Q2)

                    F2D= f22*T4F2
                
                elif self.ht==1: return self.get_T4FX('F2','deuteron',x,Q2)
                elif self.ht==2: return self.get_F2D(x,Q2,twist=2,tmc=tmc,nuc=nuc,offshell=False)*self.get_T4FX('F2','deuteron',x,Q2)
        ...
        if offshell==True:
            ...
            #--deuteron offshell smearing grids (same for proton/neutron)
            if tmc==False: 
            ...
            for nucleon in ['proton','neutron']:
                ...
                F2Doff  += np.einsum('j,jk,k->jk',TXF2,f22off,df2,dtype=complex,optimize=True)
                F2Doffc += np.einsum('j,jk,k->jk',TXF2,f22offc,df2c,dtype=complex,optimize=True)
    ...
    def get_FLD(self,x,Q2,twist=2,Nf=None,tmc=False,nuc=False,evolve=True,offshell=False):
        ...
        if offshell==False:
              ...
              if twist==4:
                  ...
                  elif self.ht==1: return self.get_T4FX('FL','deuteron',x,Q2)
                  elif self.ht==2: return self.get_FLD(x,Q2,twist=2,tmc=tmc,nuc=nuc,offshell=False)*self.get_T4FX('FL','deuteron',x,Q2)

              return self.mellin.invert(1,FLD)

        if offshell==True:

            print('ERR: No offshell corrections for FL')
            sys.exit()
    ...
    def get_F3D(self,x,Q2,twist=2,Nf=None,tmc=False,nuc=False,offshell=False,evolve=True): 
        ...
        if offshell==False:
            ...
            if twist==2:
                ...
                if    tmc==False and nuc==False: f33 = x**(-self.mellin.N)
            ...
            if twist==4:
                ...
                if self.ht==0:
                    ...
                    if nuc==False : f33==x**(-self.mellin.N)
                    else          : f33=self.tables[self.get_tname('4',fXX='f33',proj=None)].retrieve(x,Q2)

                    F3D= f33*T4F3

                elif self.ht==1: return self.get_T4FX('F3','deuteron',x,Q2)
                elif self.ht==2: return self.get_F3D(x,Q2,twist=2,tmc=tmc,nuc=nuc,offshell=False)*self.get_T4FX('F3','deuteron',x,Q2)
        ...
        if offshell==True:
            ...
            print('ERR: No offshell corrections for F3')
            sys.exit()
