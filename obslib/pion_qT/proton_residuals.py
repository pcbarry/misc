    ...
    def _get_theory(self,entry):
        ...
        if self.tabs[idx]['obs'][0] == 'd2sigma/dpTdx':
            ...
            for f in ['g','ub','db','s','sb','c','cb','b','bb']:
                conf['pdf'].storage[mu2][f]=np.zeros(len(conf['pdf'].storage[mu2][f]))
            thy1 = conf['pion_qT'].get_xsec2(pT,xF,s,Qmin,Qmax)
            del conf['pdf'].storage[mu2]
            conf['pdf'].evolve(mu2)
            ...
            Qmax  = self.tabs[idx]['Qmax'][i]
            conf['pdf'].evolve(mu2)
            conf['pdf'].storage[mu2]['g']=np.zeros(len(conf['pdf'].storage[mu2]['g']))
            conf['pdf'].storage[mu2]['u']=conf['pdf'].storage[mu2]['ub']
            conf['pdf'].storage[mu2]['d']=conf['pdf'].storage[mu2]['db']
            thy = conf['pion_qT'].get_xsec2(pT,xF,s,Qmin,Qmax)
            del conf['pdf'].storage[mu2]
