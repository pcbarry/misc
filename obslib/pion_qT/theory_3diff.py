...
class PION_QT:
    ...
    def integrand(self,uxa,y,pT,s,Q):
        ...
        #conf['pdf'].evolve(mu2)
        #fA_pion=np.array([conf['pdf'].get_xF(xa,mu2,_,evolve=False)/xa for _ in self.flav])
        ...
        jac = 1.0/(xT**2+4*tau)/np.cosh(y) #--for dsigma/dy -> dsigma/dxF
        factor = xa*xb/(xa-x1) * 2 * pT  * (1-xamin) * 2*Q * jac * 0.3893793656*1e-27
        return dsigma*factor

    def get_xsec(self,pT,s,Q,y):
        """
        returns dsig/dpTdQdx
        """
        df = lambda uxa: self.integrand(uxa,y,pT,s,Q)
        return fixed_quad(np.vectorize(df),0,1,n=10)[0]
