...
class RESIDUALS(_RESIDUALS):
    ...
    def resample_ccLCS(self):
        self.tabs=copy.deepcopy(self.original)
        for k in self.tabs:
            npts=len(self.tabs[k]['value'])
            if str(k)[0]=='1':
                self.tabs[k]['value']+=np.random.randn(npts)*self.tabs[k]['alpha']
            else:
    ...
    def _get_ccLCS_residuals(self,k):
    ...
    def get_ccLCS_residuals(self):
        ...
        for k in self.tabs:
            if str(k)[0]=='1':
                ...
                res=np.append(res,self._get_ccLCS_residuals(k))

        return res,rres,nres

    def setup_ccLCS(self):
        ...
        if 'bootstrap' in conf and conf['bootstrap']: self.resample_ccLCS()
...
if __name__=='__main__':
    ...
    #print(residuals.tabs[22781]['thy'])
    #print(residuals.tabs[23581]['thy'])
